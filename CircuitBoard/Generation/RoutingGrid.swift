...
final class RoutingGrid {
    ...
    private func dilate(_ admit: (Int, Int, Int, UInt8) -> Bool) {
        // Use local pointer variables for the swap ping-pong
        var src = frontier
        var dst = nextFrontier
        ...
        for level in 1...Routing.hugRadius {
            ...
            // Swap the local pointers for the next level
            let temp = src
            src = dst
            dst = temp
        ...
        // At the end of the dilation, frontier must always point to the last
        // valid array of sources so settleHug/rebuildHug can be called sequentially safely.
        // If we swapped an odd number of times, write the active frontier back.
        if src != frontier {
            frontier.update(from: src, count: frontCount)
        }
        frontCount = 0
    }
    ...
    func settleHug() {
        ...
        pendingCount = 0 // Clear after successful application
    }
    ...
    func rebuildHug() {
        ...
        for w in 0..<wordCount {
            var bits = occWords[w]
            while bits != 0 {
                let tz = bits.trailingZeroBitCount
                let i = (w << 6) | tz
                bits &= ~(1 << tz) // Safe bitwise clear
